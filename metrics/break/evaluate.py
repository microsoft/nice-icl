...
def get_ged_plus_scores(preds, golds, exclude_thr=None, debug=False, num_processes=5):
    decomposition_graphs = [get_decomposition_from_string(pred).to_graph() for pred in preds]
    gold_graphs = [get_decomposition_from_string(gold).to_graph() for gold in golds]
...
gts = df['answers'].tolist()

print(mp.cpu_count())
scores = get_ged_plus_scores(preds, gts, num_processes=min(mp.cpu_count(), len(preds)))
