...
tokenized_training_set = [word_tokenize(example['text']) for example in FinQA_train]
# Build BM25 index
...
for i in range(args.min_num_neighbors, args.max_num_neighbors + 1):
    ...
    for query in tqdm(FinQA_test):
        ...
        scored_documents = list(zip(scores, FinQA_train))
    ...
    df.to_csv('./outputs/{}_finqa_bm25.csv'.format(args.exec_model))
