...
train_corpus = [example[Q] for example in FinQA_train]
test_corpus = [example[Q] for example in FinQA_test]
...
for nn in range(args.min_num_neighbors, args.max_num_neighbors+1):
    ...
    for i in tqdm(range(len(dist_matrix))):
        ...
        q = FinQA_test[i][Q]
        label = FinQA_test[i][A]
                    
        train_indices = indices[i]
        train_ins = pick_elements(train_indices, FinQA_train)
    ...
    df.to_csv('./outputs/{}_finqa_topk-bert.csv'.format(args.exec_model))
