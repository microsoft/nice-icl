...
tokenized_training_set = [word_tokenize(example["text"]) for example in sst2_train]
...
for i in range(args.min_num_neighbors, args.max_num_neighbors + 1):
    ...
    for query in tqdm(sst2_test):
        ...
        scored_documents = list(zip(scores, sst2_train))
    ...
    df.to_csv('./outputs/{}_sst2_bm25.csv'.format(args.exec_model))