...
tokenized_training_set = [word_tokenize(example['text']) for example in trec_train]
...
for i in range(args.min_num_neighbors, args.max_num_neighbors + 1):
    ...
    for query in tqdm(trec_test):
        ...
        scored_documents = list(zip(scores, trec_train))
    ...
    df.to_csv('./outputs/{}_trec_bm25.csv'.format(args.exec_model))
