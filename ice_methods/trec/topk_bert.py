...
train_corpus = [example[Q] for example in trec_train]
test_corpus = [example[Q] for example in trec_test]
...
for nn in range(args.min_num_neighbors, args.max_num_neighbors+1):
    ...
    for i in tqdm(range(len(dist_matrix))):
        ...
        train_ins = pick_elements(train_indices, trec_train)
    ...
    df.to_csv('./outputs/{}_trec_topk-bert.csv'.format(args.exec_model))
