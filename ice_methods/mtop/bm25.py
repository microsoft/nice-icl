...
tokenized_training_set = [word_tokenize(example['text']) for example in mtop_train]
# Build BM25 index
...
for i in range(args.min_num_neighbors, args.max_num_neighbors+1):
    ...
    for query in tqdm(mtop_test):
        ...
        scored_documents = list(zip(scores, mtop_train))
    ...
    df.to_csv('./outputs/{}_mtop_bm25.csv'.format(args.exec_model))
