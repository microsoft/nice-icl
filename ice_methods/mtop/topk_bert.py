...
train_corpus = [example[Q] for example in mtop_train]
test_corpus = [example[Q] for example in mtop_test]
...
for nn in range(args.min_num_neighbors, args.max_num_neighbors+1):
    ...
    for i in tqdm(range(len(dist_matrix))):
        ...
        q = mtop_test[i][Q]
        label = mtop_test[i][A]
        lang = mtop_test[i]['lang']
        domain = mtop_test[i]['domain']
        
        train_indices = indices[i]
        train_ins = pick_elements(train_indices, mtop_train)
        ...
        for _ in range(args.num_permutations):
          ...
          ice_examples = ""
          for doc in train_ins:
              ice_examples += f"Input:{doc['text']}\nLanguage: {doc['lang']}\nDomain: {doc['domain']}\nOutput:{doc['label']}\n\n"
              
          prompt = f"{p}{ice_examples}Input:{q}\nLanguage: {lang}\nDomain: {domain}\nOutput:"      
    ...
    res_dict = {"instructions": instructions, "answers": ground_truth, "output": outputs}
    df = pd.DataFrame.from_dict(res_dict)
    
    df.to_csv('./outputs/{}_mtop_topk-bert.csv'.format(args.exec_model))
