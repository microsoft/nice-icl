...
train_corpus = [example[Q] for example in break_train]
test_corpus = [example[Q] for example in break_test]
...
for nn in range(args.min_num_neighbors, args.max_num_neighbors+1):
    ...
    for i in tqdm(range(len(dist_matrix))):
        ...
        q = break_test[i][Q]
        label = break_test[i][A]
                    
        train_indices = indices[i]
        train_ins = pick_elements(train_indices, break_train)
    ...
    df.to_csv('./outputs/{}_break_topk-bert.csv'.format(args.exec_model))
