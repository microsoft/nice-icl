...
prompts = []
for file in glob(os.path.join(root_dir,'instructions/medqa/*.txt')):
    ...
    for i in range(args.min_num_neighbors, args.max_num_neighbors+1):
        ...
        for query in tqdm(medqa_test):
            ...
            for _ in range(args.num_choices):
                ...
                retrieved_documents = np.random.choice(medqa_train, size=i, replace=True)
        ...
        df.to_csv('./outputs/{}_medqa_random_{}_{}.csv'.format(args.exec_model, p_name, args.label))
