...
train_corpus = [example[Q] for example in nl2bash_train]
test_corpus = [example[Q] for example in nl2bash_test]
...
for nn in range(args.min_num_neighbors, args.max_num_neighbors+1):
    ...
    for i in tqdm(range(len(dist_matrix))):
        ...
        q = nl2bash_test[i][Q]
        label = nl2bash_test[i][A]
                    
        train_indices = indices[i]
        train_ins = pick_elements(train_indices, nl2bash_train)
    ...
    df.to_csv('./outputs/{}_nl2bash_topk-bert.csv'.format(args.exec_model))
