...
prompts = []
for file in glob(os.path.join(root_dir,'instructions/nl2bash/*.txt')):
...
def call_llama2(model, tokenizer, text: str, num_tokens: int = 256, temp=0.000001):
    ...
    {"role": "system", "content" : "You are a kind and helpful assistant."}
        ...
        for query in tqdm(nl2bash_test):
            ...
            for _ in range(args.num_choices):
                ...
                retrieved_documents = np.random.choice(nl2bash_train, size=i, replace=True)
        ...
        df.to_csv('./outputs/{}_nl2bash_random_{}_{}.csv'.format(args.exec_model, p_name, args.label))
