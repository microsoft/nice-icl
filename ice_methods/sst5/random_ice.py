...
prompts = []
for file in glob(os.path.join(root_dir,'instructions/sst5/*.txt')):
  ...
  for i in range(args.min_num_neighbors, args.max_num_neighbors+1):
      ...
      for query in tqdm(sst5_test):
          ...
          for _ in range(args.num_choices):
            ...
            retrieved_documents = np.random.choice(sst5_train, size=i, replace=True)     
      ...
      df.to_csv('./outputs/{}_sst5_random_{}_{}.csv'.format(args.exec_model, p_name, args.label))
