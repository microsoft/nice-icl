...
train_corpus = [example[Q] for example in sst5_train]
test_corpus = [example[Q] for example in sst5_test]
...
for nn in range(args.min_num_neighbors, args.max_num_neighbors+1):
    ...
    for i in tqdm(range(len(dist_matrix))):
        ...
        q = sst5_test[i][Q]
        label = num2label[str(sst5_test[i][A])]
                    
        train_indices = indices[i]
        train_ins = pick_elements(train_indices, sst5_train)
    ...
    df.to_csv('./outputs/{}_sst5_topk-bert.csv'.format(args.exec_model))
